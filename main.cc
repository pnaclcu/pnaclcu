	 ...
	 //   temp2(rect1).copyTo(noparking);
		...
		//std::string name = std::to_string(num);
	        int2str(num,name);
                if(num%15==0)
	       {
	        imwrite("/root/ssd_plate_people_camera_0606/image/"+ss1+".jpg",noparking);
               // imwrite(name+".jpg",noparking);
                imwrite("/root/ssd_plate_people_camera_0606/image/"+ss1+"ori.jpg",currFrame);
               }
		//if(num%15==1)
		//{
...
const float CONF_THRESHOLD = 0.22;
const float FACE_THRSHOLD = 0.22;
const float PLATE_THRSHOLD = 0.22;
...
{  
    ...
    memset(&serv_addr, 0, sizeof(serv_addr));  //void *memset(void *s, char ch, unsigned n);初始化
    serv_addr.sin_family = AF_INET;  //tcp/ip协议,
    serv_addr.sin_port = htons(atoi("8887"));  //str>>int
    inet_pton(AF_INET, "192.168.1.100", &serv_addr.sin_addr); //int inet_pton(int family, const char *strptr, void *addrptr); 10进制IP地址转为网络传输数值格式
    //inet_pton(AF_INET, argv[1], &serv_addr.sin_addr);  
         
    /* connect the server commented by  */  
    i_ret = connect(sock_id, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr)); 
//int connect(int sockfd(客户端), const struct sockaddr *addr（服务器IP）,socklen_t addrlen（服务端字节长度）);客户端通过connect函数与服务端连接，进行通信
    ...
    bzero(buf, MAXLINE);  //buf里maxline置0


//fread（*ptr,size of elemets,num of elemets,*FILE）FILR到PTR
...
//  fprintf(stdout,"mbox_loc  size : %d\n",size_loc);   //shunyi add for debug
  ...
  vector<string> label = {"background","face","plate"};
...
//    softmax4(conf, size/4, conf_scale, conf_softmax);
    ...
    for (size_t i = 0; i < results.size(); ++i) {
      ...
      //     << results[i][5] << endl;
      if (label_index == 1 && results[i][5] > FACE_THRSHOLD) {
        time_t rawtime;
	struct tm *info;
	char buffer[128];
	time(&rawtime);
	...
	strftime(buffer,80,"%b%d_%H%M",info);
     ...
     ss1+=count_plate;
     imwrite("/root/ssd_plate_people_camera_0606/img/"+ss1+".jpg",plate);
     ss1=temp_ss1;
      ...
      } else if (label_index == 2 && results[i][5] > PLATE_THRSHOLD) {  // 
        time_t rawtime;
	struct tm *info;
	char buffer[128];
	time(&rawtime);
...
int main(int argc, char** argv) {
  ...
  while(1){
   cap >> image;
    ...
    cout<<"***********"<<"the pitcures below came from original area"<<"**********"<<endl;
    RunSSD(task_conv_1,image);
  //  dpuDestoryTask(task_conv_1);
   // dpuDestoryTask(task_conv_2);
//  dpuDestoryKernel(kernel_conv); 
    LaneDetect detect(image_bu);
   cout<<"***********"<<"the pitcures below came from noparking area"<<"**********"<<endl;
